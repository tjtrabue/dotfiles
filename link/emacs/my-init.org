...
#+begin_src emacs-lisp
  ...
  with straight.el.")
    ...
    (defconst my/line-number-state nil
      "The current global line number setting.
  Either `absolute', `relative', `visual', or nil to disable line numbers altogether.")
...
#+begin_src emacs-lisp
  ...
  ;; - 'visual: Like relative, but counts screen lines instead of buffer lines
  ;; -       nil: No line numbers
  ...
  ;; The string Emacs uses to represent arrows in various contexts.
...
#+begin_src emacs-lisp
  ...
  (use-package my-font
    ...
    :custom-face
    (default ((nil (:family "MonoLisa Variable" :height 105 :weight regular :width normal))))
    (fixed-pitch ((nil (:family "MonoLisa Variable" :height 105 :weight regular :width normal))))
...
#+begin_src emacs-lisp
  (when (display-graphic-p)
    ;; When called interactively, prompt the user for a fringe style to apply.
    ;; When called from a Lisp program, the argument must be one of:
    ;; - nil -> default fringe width of 8 pixel for both the left and right fringes.
    ;; - a cons cell (LEFT . RIGHT) -> LEFT and RIGHT are the pixel widths for the left and right
    ;;   fringes, respectively.
    ;; - a single integer -> specifies the pixel widths of both fringes.
    (fringe-mode '(12 . 8)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;;; Set universal keybindings with General
    ...
    (my/evil-leader-def
      ...
      ;; Toggle showing line numbers in the buffer
...
#+begin_src emacs-lisp
  (use-package org-auto-tangle
    ...
    :custom
    ;; Whether to use auto-tangle as the default behavior for all org buffers.  In my opinion,
    ;; tangling should always be a deliberate choice. Tangling all src blocks by default can lead to
    ;; messy directories and unexpected results.
    (org-auto-tangle-default nil))
#+end_src

** emacs-async
=async.el= is a module for asynchronous processing in Emacs. This package also
provides some async applications:
