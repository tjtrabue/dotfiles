...
#+begin_src emacs-lisp
  ...
  (use-package exec-path-from-shell
    ...
    (when (memq system-type '(gnu gnu/linux darwin cygwin))
      (let ((old-zdotdir (getenv "ZDOTDIR")))
        ;; USE_LEAN_PROFILE is set to make shell startup as minimal as possible.
        ;; We do not want this plugin to execute a fancy login shell. We only want
        ;; to get PATH from the environment.
        (setenv "USE_LEAN_PROFILE" "true")
        ;; Also need to make sure ZDOTDIR is not set so that Zsh will not detect
