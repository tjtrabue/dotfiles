...
concatenates them into one, giant Emacs Lisp file named =tjtrabue-emacs.el=. My
=~/.emacs= file knows to look for the super config file, and if it does not yet
exist, generate it using the aforementioned script.
