...
#+begin_src emacs-lisp
  ...
  This function preserves the user's current `evil-state', and will
  account for a closing paren being directly under the evil-mode
  cursor."
    (interactive)
    (evil-save-state
      ;; We need to move point ahead one column if we're in normal or visual state,
      ;; and the cursor is directly over a closing paren.
      (if (and (member evil-state '(normal visual))
...
#+begin_src emacs-lisp
  (use-package general
    ...
    (general-def
      ;; Replace an Elisp expression with its evaluated result.
