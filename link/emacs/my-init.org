...
a day. This can be discouraging, but it is nothing to worry about. Here are some
tips for ensuring Emacs does, in fact, finish compiling your Elisp files and
becomes useable:
...
- Run Emacs in daemon-mode from the command line to keep track of the native
  ...
  #+begin_src shell :tangle no
    # First, set the number of open file descriptors to a high value:
    ulimit -Sn 8192
    ulimit -Hn 65535
    # Then, run Emacs in server-mode and keep it in the terminal's foreground:
    emacs --fg-daemon
  #+end_src
