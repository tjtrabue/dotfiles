...
#+begin_src emacs-lisp
  (use-package treesit
    ...
    :if (and (>= emacs-major-version 29)
             (string-match-p "\sTREE_SITTER\s" system-configuration-features))
    :preface
    ...
    (defun my/install-missing-treesit-grammars ()
      ...
      (interactive)
      (let ((ts-grammars-dir (concat (file-truename (getenv "HOME")) "/.emacs.d/tree-sitter"))
        ...
        (if (file-directory-p ts-grammars-dir)
            ...
            ;; grammars.
            (let ((installed-grammars
                   (mapcar (lambda (f)
                             (intern
                              (string-remove-prefix "libtree-sitter-" (string-remove-suffix ".so" f))))
                           (seq-remove (lambda (f)
                                         (member f '("." "..")))
                                       (directory-files ts-grammars-dir)))))
              (dolist (g ts-grammars)
                (unless (memq g installed-grammars)
                  (treesit-install-language-grammar g))))
          ;; If the tree-sitter grammar dir does not exist, install all grammars from scratch.
          (mapc #'treesit-install-language-grammar ts-grammars))))
    :config
