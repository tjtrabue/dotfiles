...
Emacs, being one of the flagship GNU programs, has wondrous integration with
other GNU tools, not the least of which is Grep.

#+begin_src emacs-lisp
  ;; Change standard `grep-find' command to use Ripgrep when available.  The 27 means place the cursor
  ;; at the 27th character, right between the '' where we will begin typing our search expression.
  (when (executable-find "rg")
    (grep-apply-setting
     'grep-find-command
     '("rg -n -H --no-heading -e '' $(git rev-parse --show-toplevel || pwd)" . 27))
    ;; The default command to run for `M-x lgrep'.
    (grep-apply-setting 'grep-template "rg --color=always --no-heading -HPin -e <R>"))
#+end_src
...
#+begin_src emacs-lisp
  ;; Native tree-sitter support was added in Emacs 29.
  (when (functionp 'treesit-install-language-grammar)
    ...
    ;; Install language grammars defines in `treesit-language-source-alist'.
    ;; Run this on-demand whenever you change `treesit-language-source-alist'.
