...
#+begin_src emacs-lisp
  (use-package org
    ...
    :config
    ;; Add '-shell-escape' flag to all LaTeX commands that Org uses to generate previews, and make
    ;; sure that `:latex-compiler' command strings generate DVI files when appropriate.
    (let ((dvipng-default (alist-get 'dvipng org-preview-latex-process-alist))
          (dvisvgm-default (alist-get 'dvisvgm org-preview-latex-process-alist))
          (imagemagick-default (alist-get 'imagemagick org-preview-latex-process-alist)))
      (setf (alist-get 'dvipng org-preview-latex-process-alist)
            (plist-put dvipng-default :latex-compiler `(,(concat "lualatex"
                                                                 " --shell-escape"
      ...
      (setf (alist-get 'dvisvgm org-preview-latex-process-alist)
            (plist-put dvisvgm-default :latex-compiler `(,(concat "lualatex"
                                                                  " --shell-escape"
                                                                  " --interaction=nonstopmode"
                                                                  " --output-format=dvi"
                                                                  " --output-directory=%o"
                                                                  " %f"))))
      (setf (alist-get 'imagemagick org-preview-latex-process-alist)
            (plist-put imagemagick-default :latex-compiler `(,(concat "lualatex"
                                                                      " --shell-escape"
                                                                      " --interaction=nonstopmode"
                                                                      " --output-format=pdf"
                                                                      " --output-directory=%o"
