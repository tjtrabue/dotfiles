...
#+begin_src emacs-lisp
  ...
  (when (executable-find "rg")
    ;; Change standard `grep-find' command to use Ripgrep when available.  The 27 means place the
    ;; cursor at the 27th character, right between the '' where we will begin typing our search
    ;; expression.
    (setq grep-find-command
          '("rg -n -H --no-heading -e '' $(git rev-parse --show-toplevel || pwd)" . 27))
    ;; The default command to run for `M-x lgrep'.
    (setq grep-template "rg --color=always --no-heading -HPin -e <R>"))
