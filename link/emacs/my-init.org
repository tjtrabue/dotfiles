...
best way to use =delight=.

If you pass the =:delight= keyword to =use-package= without an argument,
=use-package= will hide the lighter text for the main mode provided by the
plugin by doing its best to guess the =delight= spec for you. However, this will
not work if the name of the minor mode provided by the package differs from the
name of the package. Aside from the no-argument method of using =:delight=, the
keyword behaves exactly like the =delight= function itself, although the
=use-package= keyword is slightly more forgiving.
...
#+begin_src emacs-lisp
  (use-package org
    :mode ("\\.org\\'" . org-mode)
    :delight (org-src-mode nil org-src)
    :hook
    (org-mode .
              (lambda ()
