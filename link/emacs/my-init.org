...
* TEMP FIX
  This section houses temporary fixes that should be removed once true solutions
  are added to the main Emacs project.
...
(define-advice define-obsolete-function-alias
    (:filter-args (ll) fix-define-obsolete-function-alias)
...
(define-advice define-obsolete-variable-alias
    (:filter-args (ll) fix-define-obsolete-variable-alias)
  (let ((obsolete-name (pop ll))
        (current-name (pop ll))
        (when (if ll (pop ll) "1"))
        (docstring (if ll (pop ll) nil)))
...
(define-advice make-obsolete
    (:filter-args (ll) fix-make-obsolete)
  (let ((obsolete-name (pop ll))
        (current-name (pop ll))
        (when (if ll (pop ll) "1")))
...
(define-advice make-obsolete-variable
    (:filter-args (ll) fix-make-obsolete-variable)
  (let ((obsolete-name (pop ll))
        (current-name (pop ll))
        (when (if ll (pop ll) "1"))
        (docstring (if ll (pop ll) nil)))
