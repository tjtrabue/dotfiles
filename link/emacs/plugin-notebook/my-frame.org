...
* Moom
  ...
  Emacs. =moom= was a macOS tool used for easily adjusting GUI windows /ala/ tiling
  window managers, and this plugin is a port of =moom= to Emacs.
...
** *NOTE:* You probably don't want =moom= on Linux
  ...
  begin with. =moom= can be quite useful on macOS computers, though, so I've added
  some configuration for Mac users. However, macOS computers /do/ have access to
