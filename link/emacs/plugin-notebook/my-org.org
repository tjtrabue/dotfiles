  ...
  (use-package org-modern
    ...
    (org-modern-checkbox
     '((?X . "✔")
       (?- . "┅")
       (?\s . " ")))
    ;; Update the org-sticky-header bullet character to match org-modern bullets.
    (org-sticky-header-heading-star "▶")
    ;; Edit basic Org settings for better compatibility with `org-modern'.
...
=org-modern= provides a clean and efficient org style. The /blocks/ (e.g. source,
example) are particularly nicely decorated. However, enabling =org-indent=
disables the block "bracket", which uses the fringe area. This small package
approximately reproduces the block styling of =org-modern= when using
=org-indent=. Emacs users may enable =org-modern-indent= with or without =org-modern=.
...
#+begin_src emacs-lisp
  (use-package org-tree-slide
    ;; Re-enable when the need for org-tree-slide arises.
    ;; Otherwise, this just slows down Emacs' startup time.
    :disabled
    :general
...
#+begin_src emacs-lisp
  (use-package ox-gfm
    :hook
    (org-load . (lambda ()
                  ;; Support GitHub Flavored Markdown in Org's export feature.
...
#+begin_src emacs-lisp
  (use-package org-ql
    ;; I do not currently use this plugin.
    ;; Enable if desired.
    :disabled
    :after org)
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-sticky-header
    ...
    ;; The star, or bullet character that prefixes the sticky heading
    (org-sticky-header-heading-star "▶"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-web-tools
    ;; This package is helpful if you want to download entire web pages as Org mode files, a
    ;; specialized use case for which I rarely have need.
    ;; Enable if desired.
    :disabled)
#+end_src
