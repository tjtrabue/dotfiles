...
#+begin_src emacs-lisp
  ...
  (defun my/cider-insert-last-sexp-in-repl (&optional arg)
    "Advances point one character forward before inserting the last s-exp in the REPL.

  This properly integrates `evil-mode' with CIDER since, while in normal mode, Emacs assumes point
  is one character behind the cursor rather than on the cursor, leading to undesirable behavior
  when executing `cider-insert-last-sexp-in-repl'.

  If invoked with prefix ARG, evaluate the s-expression in the REPL."
    (interactive "P")
    (save-excursion
      (forward-char)
...
#+begin_src emacs-lisp
  (use-package cider
    ...
    (my/evil-leader-def cider-mode-map
      ...
      "ci"  'my/cider-insert-last-sexp-in-repl
      "cjj" 'cider-jack-in
