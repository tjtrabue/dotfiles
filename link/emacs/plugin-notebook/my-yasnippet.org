...
snippet, followed by a /trigger/ for the snippet, which is a key or key chord that
expands the snippet based on the abbreviation before point.
...
#+begin_src emacs-lisp
  (use-package yasnippet
    ...
    ;; If non-nil, insert region contents for a snippet's $0 field.
...
#+begin_src emacs-lisp
  (use-package yasnippet-snippets
    :after (yasnippet common-lisp-snippets clojure-snippets)
    :demand t
    :config
    (yas-reload-all))
#+end_src
...
#+begin_src emacs-lisp
  (use-package helm-c-yasnippet
    ;; I prefer not to use helm if at all possible.
    :disabled
    :after yasnippet
    :general
    (general-def 'global
