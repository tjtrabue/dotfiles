...
#+begin_src emacs-lisp
  (use-package vertico
    ...
    ;; Configure the display per com pletion category.
    (vertico-multiform-categories
     ;; Use a buffer for the consult-grep commands.
     '((consult-grep buffer)))
    :init
...
#+begin_src emacs-lisp
  (use-package vertico-posframe
    ...
    :init
    (add-to-list 'vertico-multiform-commands '(t posframe))
    (add-to-list 'vertico-multiform-commands
                 '(consult-line
                   posframe
                   (vertico-posframe-poshandler . posframe-poshandler-frame-top-center)
                   (vertico-posframe-border-width . 10)
                   ;; NOTE: This is useful when emacs is used in both in X and terminal since posframe
                   ;; does not work well in terminal, so `vertico-buffer-mode' will be used as the
