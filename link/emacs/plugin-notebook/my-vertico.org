...
#+begin_src emacs-lisp
  (use-package vertico
    ...
    ;; NOTE: You must set this variable to t if using the `vertico-posframe' plugin.
    (vertico-resize t)
    ...
    ;;  ;; Use a verbose, separate buffer for the following commands
    ;;  '((consult-grep buffer)
    ;;    (imenu buffer indexed)))
    ;; NOTE 3/27/25: Temporarily disabling configured multiform commands and categories due to a
    ;; Vertico bug that must be fixed. Try uncommenting the above lines after a few weeks.
