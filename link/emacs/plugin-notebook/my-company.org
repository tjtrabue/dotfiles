...
#+begin_src emacs-lisp :tangle yes
  (use-package company-plsense
    ...
    ((cperl-mode perl-mode) . (lambda ()
      ...
      ;; Make sure the buffer gets initialized for company-plsense.
...
#+begin_src emacs-lisp :tangle yes
  (defvar my/modes-for-company-lsp '(
    cperl-mode
    perl-mode
    sh-mode)
  "This is a list of major modes to associate with the
...
#+begin_src emacs-lisp :tangle yes
(defun my/add-company-lsp-hooks ()
  "Add hooks to specific major modes for using company-lsp."
    (mapc #'(lambda (mode)
        (lexical-let ((mode mode)
                      (mode-hook-name (concat (symbol-name mode) "-hook")))
          (add-hook (intern mode-hook-name) #'(lambda ()
            (make-local-variable 'company-backends)
            (add-to-list 'company-backends 'company-lsp)))))
      my/modes-for-company-lsp))
#+end_src
