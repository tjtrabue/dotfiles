...
#+begin_src emacs-lisp
  (use-package sly
    ...
    ;; Extra SLY features to load. Check the contrib/ subdirectory of the sly repo
    ...
    (sly-contribs (delete-dups (append '(sly-autodoc
                                       ...
                                       sly-contribs)))
    :init
    ;; Add the SLY contrib packages to Emacs' load-path.
...
will work! Otherwise, you'll To do so, simply run:
