...
#+begin_src emacs-lisp
  (use-package markdown-mode
    ...
    ;; Whether to hide markup punctuation such as the delimiters for bold **...**, italics _..._,
    ;; monospace `...`, etc.
...
#+begin_src emacs-lisp
  (use-package grip-mode
    ...
    :general
    (my/user-leader-def '(markdown-mode-map org-mode-map)
      ;; Set keybindings for functions listed in the :commands block.
      "G b" 'grip-browse-preview
      "G r" 'grip-restart-preview
      "G s" 'grip-start-preview
      "G S" 'grip-stop-preview)
    :custom
    ...
    ;; Use `mdopen' Rust CLI tool, if available, instead of `grip'. `mdopen' is nice because it is
    ;; fast, but better yet, it circumvents accessing GitHub's API to render the document preview.
    ;; NOTE: `mdopen' requires the use of an external browser, and does not respect Emacs' webkit
    ;;       preview.
    ;; (grip-use-mdopen (executable-find "mdopen"))
    (grip-use-mdopen nil)
