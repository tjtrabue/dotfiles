...
#+begin_src emacs-lisp
  (use-package evil
    :demand t
    :hook
    ;; If we are displaying line numbers in the current buffer, activate relative line numbers by
    ;; default, and absolute line numbers when in evil's insert state.
    (evil-insert-state-entry . (lambda ()
                                 (when (bound-and-true-p display-line-numbers)
                                   (setq display-line-numbers t))))
    (evil-insert-state-exit . (lambda ()
                                (when (bound-and-true-p display-line-numbers)
                                  (setq display-line-numbers (if (eq my/line-number-state 'absolute)
                                                                 t
                                                               'relative)))))
    :general
    ...
    ;; Whether movement commands respect `visual-line-mode'. If non-nil, movement commands, such as j
    ;; and k, navigate by visual lines (on the screen) rather than "physical" lines (defined by
    ;; newline characters). If nil, the setting of `visual-line-mode' is ignored.
...
#+begin_src emacs-lisp
  (use-package evil-leader
    ...
    ;; Save all buffers visiting files with :W<return>
    (evil-define-command W
      "Write all buffers visiting files."
      (save-some-buffers t)))
#+end_src
...
** evil-matchit
Port of =vim-matchit= for =evil-mode=. =evil-matchit= extends the VI =%= operator by
adding programming language aware delimiter matching (for instance, jumping
between matching =<div></div>= tags in HTML). HTML, Python, Java, Bash, Ruby,
Elixit, Markdown, and YAML are just a few of the languages =evil-matchit=
supports.

As an interesting aside, Evil is not required to use =evil-matchit=. If you forego
Evil in favor of Emacs' default keybindings, you must bind
=evilmi-jump-items-native= to some custom key sequence.
...
consecutive motions beginning with =gx=. For instance, to exchange the words "red"
and "blue" in the following stanza, place point behind "red", type =gx w=, then
move point behind "blue", and again type =gx w=:

#+begin_src :tangle no
  Roses are <gx w>red,
  Violets are <gx w>blue
#+end_src

How useful is this feature? I will leave that decision to the user. In general,
Vim's editing features are text-oriented, and have little understanding of code
semantics. Emacs, on the other hand, favors plugins that have code-aware editing
capabilities, so the two editors' philosophies are quite different. I believe
that plugins like =evil-exchange= exist to satisfy Vim expats who miss their old
editor plugins. I, however, like to lean on Emacs' more powerful features, so I
find little use for basic text manipulation plugins.

#+begin_src emacs-lisp
  (use-package evil-exchange
    ;; I have never found much use for this plugin.
    :disabled
    :after evil-collection
    :demand t
    :config
...
** evil-visualstar
An Emacs port of one of the many visual-star plugins for Vim. When
=evil-visualstart-mode= is active, make a visual selection with =v= or =V= then
hit =*= to search forward or =#= to search backward.
...
#+begin_src emacs-lisp
  (use-package evil-snipe
    ;; I have not used `evil-snipe' in a long time, and to be honest, Avy provides a much more
    ;; powerful system for fast, flexible text movement.
    :disabled
    :demand t
    :after evil-collection
...
#+begin_src emacs-lisp
  (use-package evil-multiedit
    ...
    (general-def '(normal visual)
      ...
      ;; We don't need `zap-to-char' if using evil-mode
      "M-z" 'evil-multiedit-match-all)
    (general-def 'insert
...
** evil-cleverparens
A delimiter-balancing plugin for =evil-mode=. A bit like =smartparens=.

#+begin_src emacs-lisp
  (use-package evil-cleverparens
    ;; Lispyville is superior and integrates with Lispy.
    :disabled)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun my/lispyville-duplicate-at-point (count)
    "Duplicates the symbol or s-exp at point COUNT times."
    (interactive "P")
    (save-excursion
...
#+begin_src emacs-lisp
  (use-package lispyville
    ...
    (general-def lispyville-mode-map
      "C-S-d" 'lispyville-delete-whole-line
    ...
    (general-def 'normal lispyville-mode-map
      ...
      ;; Remap evil-nerd-commenter commands to lispyville versions.
      [remap evilnc-comment-or-uncomment-lines] 'lispyville-comment-or-uncomment-line)
    ...
    (general-def '(normal visual) lispyville-mode-map
      ...
      "<<" 'lispyville-<
      ">>" 'lispyville->
      ;; Clone the s-exp at point
      "C-y" 'my/lispyville-duplicate-at-point
