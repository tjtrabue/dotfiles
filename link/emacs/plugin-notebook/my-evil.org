...
Vim's editing features are text-oriented, and have little in the way of semantic
code understanding. Emacs, on the other hand, favors plugins that enable code
awareness in editing capabilities, so the two editors' philophies are quite
different. I believe that plugins like =evil-exchange= exist to satisfy Vim expats
who miss their old editor plugins. I, however, like to lean on Emacs' more
powerful features, so I find little use for basic text manipulation plugins.
