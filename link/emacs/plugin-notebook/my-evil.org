...
*** Functions
    #+begin_src emacs-lisp :tangle yes
      ...
      (defun my/lispy-forward-evil-normal ()
        "A variant of `lispy-forward' that words in `evil-normal-state'."
        (interactive)
        (let ((char-at-point (char-after (point))))
          (when (eq ?\) char-at-point)
            (forward-char 1))
          (lispy-forward 1)))
    #+end_src
   ...
   #+begin_src emacs-lisp :tangle yes
     (use-package lispyville
       ...
       (:map evil-collection-lispy-mode-map
             ...
             ("<normal-state> <" . nil)
             ("<normal-state> >" . nil)
             ;; We want '(' and ')' to navigate s-exps.
             ("<normal-state> (" . lispy-backward)
             ("<normal-state> )" . my/lispy-forward-evil-normal))
       :config
