...
** Dependencies
Magit requires the following packages to function.

#+begin_src emacs-lisp
  ;; Forwards compatibility library for Emacs, providing definitions for functions
  ;; introduced in newer versions of the editor.
  (use-package compat)

  ;; Provides the signature Magit-style UI for executing interactive Git commands.
  ;; This package used to be part of Magit, but was deemed useful enough to become
  ;; a standalone library for any plugin.
  (use-package transient)

  ;; Provides a macro `##' for terse lambda syntax:
  ;;   (## FN &rest BODY)
  ;; Kind of like #() in Clojure.
  (use-package llama)

  ;; Use `emacsclient' as the $EDITOR of child processes. Essentially provides more
  ;; sophisticated versions of `shell-command' and `async-shell-command'.
...
#+begin_src emacs-lisp
  (use-package magit-todos
    ;; Currently erroring out when `magit-status' is invoked due to bad arguments to `pcre2el' library
    ;; functions.
    :disabled
    :after magit
    :hook
...
* blamer
A git blame plugin inspired by VS Code's GitLens plugin. =blamer= shows =git blame=
previews for the current or selected lines after a brief idle period.

#+begin_src emacs-lisp
  (use-package blamer
    :general
    (my/evil-leader-def 'global
      "gi" 'blamer-show-commit-info
      "gI" 'blamer-show-posframe-commit-info)
    :custom
    (blamer-idle-time 0.3)
    (blamer-min-offset 60)
    ;; Whether blamer runs blame jobs in separate threads.
    ;; Setting this to `t' will improve blamer's performance.
    (blamer-enable-async-execution-p t)
    ;; How blamer displays messages.
    (blamer-type 'both)
    ;; Where blamer will position the overlay popup.
    ;; Can be one of:
    ;;   'bottom
    ;;   'top
    ;;   'smart - will choose the better position, usually the center
    ...
    (blamer-face ((t
                   ...
                   :italic t)))
    :config
    ;; Make sure that git-blame outputs dates in a format that blamer can parse.
    (setq blamer--git-blame-cmd '("blame" "--date=iso" "-L"))
    ;; You can enable blamer globally this way, but I prefer to enable it
    ;; as-needed by activating `blamer-mode' interactively since blamer can make
    ;; Emacs a bit sluggish.
