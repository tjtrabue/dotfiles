...
- =eval-sexp-fu-flash-mode=: Global minor mode that flashes an evaluated
  s-expression (i.e., using =C-x C-e=). Toggle EvalSexpFuFlash mode on or off. If
  this mode is on, some `eval-last-sexp'-ish commands will highlight the sexps
  during evaluation.

- =turn-on-eval-sexp-fu-flash-mode=: Unequivocally turn on EvalSexpFuFlash mode

- =eval-sexp-fu-eval-sexp-inner-list=: Evaluate the list _currently_ pointed at as
  sexp; print value in minibuffer.

- =eval-sexp-fu-eval-sexp-inner-sexp=: Evaluate the sexp _currently_ pointed; print
  value in minibuffer.

** Customizable Options:

Below are all customizable options:

- =eval-sexp-fu-flash-face=: *Face to use for showing the sexps' overlay during the evaluation.
   default = (quote eval-sexp-fu-flash)

- =eval-sexp-fu-flash-error-face=: *Face to use for showing the sexps' overlay if
  the evaluation signaled any error. The error highlighting is take into account
  only if non-nil value.
  default = (quote eval-sexp-fu-flash-error)

- =eval-sexp-fu-flash-duration=: *For time duration, highlight the evaluating
  sexps.
  default = 0.15
...
- =eval-sexp-fu-flash-function=: *Function to be used to create all of the actual
  flashing implementations.
  default = (quote eval-sexp-fu-flash-default)

- =eval-sexp-fu-overlay-priority=: Priority of the overlays created by
  esf-hl-highlight-bounds.
  default = 0

- =eval-sexp-fu-flash-doit-function=: *Function to use for flashing the sexps.
...
* paredit
The ancestor of all structural editing plugins in Emacs.

Check out the [[http://danmidwood.com/content/2014/11/21/animated-paredit.html][animated tutorial]] for a great introduction to structural editing
in Paredit.

#+begin_src emacs-lisp
  (use-package paredit
    ;; Currently using Lispy and Lispyville
    :disabled
    :hook
    ((cider-repl-mode
      clojure-mode
...
** IMPORTANT: Currently using enzuru/lispy GitHub fork
abo-abo, the author of Lispy, has been MIA for two years at the time of this
writing. In his absence, GitHub user enzuru has [[https://github.com/enzuru/lispy][forked abo-abo's repo]] and is
maintaining it for the foreseeable future. Please check both GitHub repositories
for updates as to the status of the Lispy project. Also see [[https://github.com/abo-abo/lispy/issues/684][GitHub issue #684 in
abo-abo/lispy]] for more details.

** =use-package= specification
#+begin_src emacs-lisp
  (use-package lispy
    ;; 1/3/25 - Abo Abo has been absent from his GitHub account for over two years at this point.  A
    ;; good Samaritan going by the moniker Enzuru has graciously decided to fork and take over Lispy
    ;; for Abo Abo until he returns to his projects. Thus, for the time being, we should clone
    ;; Enzuru's fork of Lispy, not Abo Abo's. We should remove this `:straight' directive once
    ;; Abo Abo returns.
    :straight
    (lispy :type git :host github :repo "enzuru/lispy" :branch "master")
    :delight
    :hook
    ...
    (lispy-compat '(cider magit-blame-mode))
    :init
...
* symex
A modern replacement for Lispy that is somehow even more opinionated. Symex has
some integration with Treesitter, which will allow its editing features to
transcend Lisp dialects, making it more general-purpose than Lispy.

#+begin_src emacs-lisp
  (use-package symex
    ;; So far, I don't see anything in Symex I don't already get from Lispy.
    :disabled
    :demand t
    :delight
    :custom
