...
#+begin_src emacs-lisp
  ...
  The car of each cons cell can either be a single major mode
  symbol, or a list of major mode symbols, in which case the
  associated formatter will be applied to each major mode in the
  list.")
#+end_src

** Functions
#+begin_src emacs-lisp
  (defun my/apheleia-add-formatters (mode-formatters-alist)
    "Add apheleia formatters based on MODE-FORMATTERS-ALIST.

  MODE-FORMATTERS-ALIST maps major modes or strings representing
  file patterns to custom formatters, which are themselves alists
  mapping a formatter name to a formatter specification. See
  `apheleia-formatters' for more details on how to construct custom
  formatter specifications."
    (dolist (mode-formatter-pair mode-formatters-alist)
...
#+begin_src emacs-lisp
  (use-package apheleia
    ...
    :config
    (my/apheleia-add-formatters my/apheleia-mode-formatters-alist))
#+end_src
