...
#+begin_src shell :tangle no
  python3 -m pip install --user --upgrade epc orjson sexpdata six setuptools paramiko rapidfuzz watchdog
#+end_src
...
#+begin_src emacs-lisp
  (use-package lsp-bridge
    ...
    ;; The Python 3 executable path. If you use a python distribution installed with `pyenv', it is
    ;; important that you specify the entire path to the versioned python3 executable, not the the
    ;; path to the `pyenv' shim.
    (lsp-bridge-python-command (if (executable-find "pyenv")
                                   (string-trim
    ...
    (add-hook 'lsp-bridge-mode-hook
              (lambda ()
                ...
                (when (bound-and-true-p lsp-bridge-mode)
                  ...
                  ;; Disable flycheck-pos-tip-mode since it interferes with lsp-bridge's diagnostic
                  ;; UI.
