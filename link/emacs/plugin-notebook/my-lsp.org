...
* Register New LSP Servers
  ...
  #+begin_src emacs-lisp :tangle yes
    (defun my/register-lsp-servers ()
      ...
      (my/register-common-lisp-lsp-servers))
  #+end_src
...
** Common Lisp
   =cl-lsp= is not fully functional yet, so if we want to use it in =lsp-mode=
   we'll need to register it ourselves.

   #+begin_src emacs-lisp :tangle yes
     (defun my/register-common-lisp-lsp-servers ()
