...
#+begin_src emacs-lisp
  (use-package modus-themes
    ;; NOTE: 1/11/2024 - Having some issues with the default straight recipe for modus-themes, which
    ;; uses the SourceHut master repo, so I'm overwriting it to use the GitHub repo, instead.  Once
    ;; SourceHut is working again, I would remove this `:straight' directive and revert back to using
    ;; SourceHut.
