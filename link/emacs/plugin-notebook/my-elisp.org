...
#+begin_src emacs-lisp
  (use-package eros
    ...
    :preface
    (defun my/eros-eval-last-sexp (arg)
      "Integrate EROS's `eros-eval-last-sexp' with Evil Mode."
      (interactive "P")
      (save-excursion
        (evil-save-state
          ...
          (evil-with-state 'insert
            (eros-eval-last-sexp arg)))))
    :general
    (general-def 'normal 'global
      "C-x C-e" 'my/eros-eval-last-sexp)
    (my/evil-leader-def
      "<"  'my/eros-eval-last-sexp
      "ee" 'my/eros-eval-last-sexp))
#+end_src
