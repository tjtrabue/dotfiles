...
#+begin_src emacs-lisp
  (use-package restclient
    ...
    (restclient :type git
                ...
                ;; For some reason, the default recipe for restclient.el does not
                ;; include the restclient-helm or restclient-jq libs, so we have
                ;; to override the file list here.
                :files (:defaults))
    ;; Treat files with `.restclient' extension as restclient query sheets.
    :mode ("\\.restclient\\'" . restclient-mode)
    :config
    ;; Needed for parsing JSON responses to piece together variables.
