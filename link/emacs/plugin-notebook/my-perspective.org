  ...
  (use-package perspective
    ...
    ((kill-emacs . persp-state-save)
     (my-tab-line-mode . (lambda ()
                           ...
                           (setq my-tab-line-tabs-function #'my/persp-tab-line-tabs-function))))
    :preface
    (require 'perspective)
    ...
    (defun my/persp-tab-line-tabs-function ()
      "Set the tab line's buffer list to a filtered list of buffers in the current perspective.

  Return the determined buffer list.
      ...
      (let* ((old-buffers (window-parameter nil 'tab-line-buffers))
             (buffer-positions (let ((index-table (make-hash-table
                                 ...
                                 (seq-do-indexed
                                  ...
                                  old-buffers)
                                 index-table))
             (new-buffers (sort (if (bound-and-true-p persp-mode)
                                    (my-tab-line-filter-display-buffers (persp-get-buffers (persp-current-name)))
                                  (my-tab-line-tab-buffers))
                                :in-place t
                                :key (lambda (buffer)
    ...
    ;; Three strings representing the opening, closing, and dividing symbols for Perspective's mode
    ;; line information.
