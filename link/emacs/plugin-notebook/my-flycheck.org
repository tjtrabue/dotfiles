...
#+begin_src emacs-lisp
  (use-package flycheck-clj-kondo
    :after flycheck
    :hook
    ((clojure-mode clojure-ts-mode) . (lambda ()
...
#+begin_src emacs-lisp
  (use-package flycheck-color-mode-line
    ;; Re-enable if using `doom-modeline'.
    :disabled
...
#+begin_src emacs-lisp
  (use-package flycheck-inline
    ;; Seems to be overridden by Embark and other plugins that output to the echo area.
    :disabled
    :after flycheck
    :hook
    (flycheck-mode . (lambda ()
                       ;; Do not activate for `lsp-mode' or `lsp-brdige-mode' due to them having their
                       ;; own dedicated diagnostic UIs.
...
#+begin_src emacs-lisp
  (use-package flycheck-pos-tip
    ;; Currently using `sideline' to display Flycheck diagnostics.
    :disabled
    :after flycheck
    :hook
    ((flycheck-mode . (lambda ()
                        ;; Put errors in tooltips if we're not in lsp-mode or lsp-bridge-mode, since
                        ;; those modes present diagnostic messages through their own UIs.
                        (unless (or (bound-and-true-p lsp-mode)
                                    (bound-and-true-p lsp-bridge-mode))
                          (flycheck-pos-tip-mode 1))))
     ((lsp-mode lsp-bridge-mode) . (lambda ()
                                     ;; Also check for disabling flycheck-pos-tip-mode when entering
                                     ;; LSP modes.
                                     (when (bound-and-true-p flycheck-pos-tip-mode)
                                       (flycheck-pos-tip-mode -1))))))
#+end_src
